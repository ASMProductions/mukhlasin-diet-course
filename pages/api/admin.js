...
// action=list     -> returns all pending AND approved items across testimonials, fast, ramadan
// action=approve  -> moves an item from pending to approved
// action=reject   -> removes an item from pending without approving it
// action=delete   -> permanently removes an item from the approved (live) list
...
export default async function handler(req, res) {
  ...
  if (req.method === "GET" && req.query.action === "list") {
    const [testimonials, fast, ramadan, testimonialsApproved, fastApproved, ramadanApproved] = await Promise.all([
      redisGet("testimonials:pending"),
      redisGet(`fastcomments:pending:${monthKey}`),
      redisGet(`ramadancomments:pending:${hijriYear}`),
      redisGet("testimonials:approved"),
      redisGet(`fastcomments:approved:${monthKey}`),
      redisGet(`ramadancomments:approved:${hijriYear}`),
    ]);
    return res.status(200).json({
      testimonials, fast, ramadan,
      testimonialsApproved, fastApproved, ramadanApproved,
      monthKey, hijriYear,
    });
  }
  ...
  if (req.method === "POST") {
    ...
    if (action === "delete") {
      ...
      const remaining = approved.filter((p) => p.id !== id);
      await redisSet(approvedKey, remaining);
